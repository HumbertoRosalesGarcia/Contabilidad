                ...
                foreach ($tc in $json.tool_calls) {
                    if ($tc.function.name -eq "default_api:write_to_file") {
                        ...
                        if ($argsJson -and $argsJson.TargetFile -and $argsJson.CodeContent) {
                            ...
                            if ($targetFile -match "FinanceScreen.kt") {
                                $code = $code -replace "    `}\r?\n`}$", "        if (showAddDialog) { com.xxcamixx.contabilidad.ui.dialogs.AddTransactionDialog(categories = viewModel.customCategories.toList(), onDismiss = { showAddDialog = false }, onConfirm = { desc, amount, isInc, note, method, cat, uri -> viewModel.addTransaction(desc, amount, isInc, note, method, cat, uri); showAddDialog = false }) }`n    }`n}"
                            }
                            
                            # 6. Global blue color replacement for UI files
                            if ($targetFile -match "\\ui\\") {
                                $code = $code -replace '0xFF4CAF50', '0xFF2196F3'
                            }

                            Set-Content -Path $targetFile -Value $code -Encoding UTF8
                            Write-Output "Restored and fixed $targetFile"
                        }
                    }
                }
            }
